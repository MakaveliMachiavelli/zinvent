...
export interface GovRates {
  year: number;
  sss: { min: number; max: number; eeRate: number; erRate: number; };
  philhealth: { rate: number; ceiling: number; };
  pagibig: { rate: number; ceiling: number; };
  taxTables: TaxTable[];
}
...
// --- Generic types for remaining products ---
...
export function isProductId(value: string): value is ProductId {
  ...
  return valid.includes(value as ProductId);
}

export function getStorageKey(product: ProductId): string {
